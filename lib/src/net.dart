...
class Connection {
  ...
  Connection(String this._db, String this._host, int this._port, String this._auth_key, {Map ssl}) {
    if (ssl != null) {
      this._ssl = ssl;
    }
  }
  ...
  Future<Connection> reconnect([bool noreply_wait = true]) {
    ...
    if (_listeners["connect"] != null) _listeners["connect"].forEach((func) => func());

    if (_ssl != null && _ssl.containsKey("caCerts")) {
      SecureSocket.connect(_host, _port, certificateName: _ssl["caCerts"], onBadCertificate: (error) {
        throw new RqlDriverError("Could not connect to $_host:$_port. SSL Certificate Error $error");
  ...
  _handleAuthResponse(List<int> res) {
    ...
    if (responseString != "SUCCESS") {
      ...
      if (_listeners["error"] != null) _listeners["error"].forEach((func) => func(error));
  ...
  _handleQueryResponse(Response response) {
    ...
    if (response._type == p.Response_ResponseType.SUCCESS_PARTIAL.value) {
      ...
      if (!query._queryCompleter.isCompleted) query._queryCompleter.completeError(new RqlDriverError("Error: ${response._data}."));
    }

    if (response._profile != null) value = {"value": value, "profile": response._profile};
    if (!query._queryCompleter.isCompleted) query._queryCompleter.complete(value);
  }

  close([bool noreply_wait = true]) {
    if (_listeners["close"] != null) _listeners["close"].forEach((func) => func());
  ...
  void addListener(String key, Function val) {
    ...
    if (_listeners != null && _listeners[key] != null) _listeners[key].forEach((element) => currentListeners.add(element));
  ...
  _handle_cursor_response(Response response) {
    ...
    if (response._type != p.Response_ResponseType.SUCCESS_PARTIAL.value && cursor._outstanding_requests == 0) _replyQueries[response._token]._cursor = null;
  }
  ...
  _read_response(res) {
    ...
    if (_response_length >= 12) {
      ...
      if (_response_length >= response_len + 12) {
        ...
        if (_replyQueries.containsKey(response._token)) _handleQueryResponse(response);
        else {
